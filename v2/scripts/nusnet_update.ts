  ...
  parse(
    ...
    async (error, result: Data[]) => {
      ...
      for (const user of result) {
        ...
        const data = (await (await fetch(`/* take from site*/`)).json()) as Body;
        ...
        if (result[0].DisplayName.toLowerCase() !== user["Name Preferred"].toLowerCase()) {
          console.error(`Name mismatch: ${user["Name Preferred"]} AND ${result[0].DisplayName}`);
        }
