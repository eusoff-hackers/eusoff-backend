...
async function compute(bid: iRoomBid, session: ClientSession) {
  ...
  const block = await RoomBlock.findOne({ block: room?.block }).session(session);
  if (!room || !block) {
  ...
  console.log(`Allocating ${room!.block}${room.number} to ${(user as iUser).username}`);
  room!.occupancy += 1;
  await room!.save({ session });
  block.quota -= 1;
  await block.save({ session });
  await RoomBidInfo.findOneAndUpdate({ user: user!._id }, { isAllocated: true, room: room!._id }).session(session);
}
...
(async () => {
  ...
  try {
    const rooms = await Room.find().session(session).populate({ path: `bidders`, populate: `info` });
    ...
    const bids = (await RoomBid.find().session(session).populate(`room`).populate(`info`).populate(`user`)).sort(
      (a, b) => b.info!.points - a.info!.points,
    );
